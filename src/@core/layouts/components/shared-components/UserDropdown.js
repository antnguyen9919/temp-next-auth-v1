        ...
        <Box sx={{ pt: 2, pb: 3, px: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            >
              <Avatar
                alt='profile_picture'
                src={user && user.profile_pic ? user.profile_pic : '/images/avatars/1.png'}
                sx={{ width: '2.5rem', height: '2.5rem' }}
              />
            </Badge>
            <Box sx={{ display: 'flex', ml: 3, alignItems: 'flex-start', flexDirection: 'column' }}>
              <Typography sx={{ fontWeight: 600 }}>{user && user.fullName ? user.fullName : 'John Doe'}</Typography>
              <Typography variant='body2' sx={{ fontSize: '0.8rem', color: 'text.disabled' }}>
                {user && user.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : null}
              </Typography>
              <Typography variant='body2' sx={{ fontSize: '0.8rem', color: 'text.disabled' }}>
                {user.org && user.org.charAt(0).toUpperCase() + user.org.slice(1)}
              </Typography>
            </Box>
          </Box>
        </Box>
